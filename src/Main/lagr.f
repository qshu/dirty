...
*
      ...
      REAL*8  R2(NMAX),R3(NMAX),RSNGL(NMAX),RBIN(NMAX),C(3)
      REAL*8  FLAGR(NLENS),RLAGR(NLENS),AVMASS(NLENS)
      REAL*8  RSLAGR(NLENS),RBLAGR(NLENS),AVMRC
      REAL*8  SIGR2(NLENS),SIGT2(NLENS),SIG2(NLENS)
      REAL*8  SIGR2C,SIGT2C,SIG2C
      REAL*8  VROT(NLENS),VROTC,RNLAGR(NLENS)
      ...
      INTEGER ISLIST(NMAX),IBLIST(NMAX),INLIST(NMAX)
*
...
*     Set square radii of resolved binaries
     ...
     &                 (X(3,I) - C(3))**2
                      ...
                      INP = INP + 1
                      R3(INP)= (X(1,I) - C(1))**2 + (X(2,I) - C(2))**2 +
     &                 (X(3,I) - C(3))**2
                      INLIST(INP) = I
                  end if
              ELSE
...
*     Sort square distances of all particles with respect to the centre C.
          ...
          IF(INP.GT.1)CALL SORT1(INP,R3,INLIST)
...
*     @qs
*  Determine the Lagrangian radii for massless particles by numbers.
*     RNLAGR = Lagrangian radius
          ZM = 0.0D0
          I = 0
*
          DO 151 J = 1, NLENS
 150      I = I + 1
          IM = INLIST(I)
          ZM = ZM + 1.D0
          IF (I.LT.nmass.AND.ZM.LT.FLAGR(J)*float(nmass)) GOTO 150
          RNLAGR(J) = SQRT(R3(I))
 151     CONTINUE
*
...
*        @qs
                  IF (nmass.gt.0) THEN
                      WRITE (6,441) TTOT, (RNLAGR(K),K=1,NLENS)
 441            FORMAT (3X,D12.4,' PLLAGR: ',1P,18(1X,D9.2))
                  END IF
          ...
          if(rank.eq.0)then
              IF (KZ(7).GE.2) THEN
                  IF(kstart.eq.1.and.time.eq.0.0) then
                      write (7,50) 2,3+NLENS,3+2*NLENS,3+3*NLENS,
     &              4+4*NLENS,
